...
const MAX_MB = 4; // Vercel serverless body limit is ~4.5MB; base64 inflates ~33%.
...
export default function Upload({ onAnalyzed }) {
  ...
  async function handleFile(file) {
    ...
    if (file.size > MAX_MB * 1024 * 1024) {
      setError(`That file is ${(file.size / 1024 / 1024).toFixed(1)} MB. Please upload a PDF under ${MAX_MB} MB (most text reports are well under this).`);
      return;
    }
    setBusy(true);
    ...
    <div style={{ maxWidth: 620, margin: "0 auto" }}>
      ...
      >
        ...
        {busy ? (
          ...
          <div>
            ...
            <p style={{ color: "rgba(255,255,255,0.4)", fontSize: "0.85rem", margin: 0 }}>or click to browse · PDF up to {MAX_MB} MB</p>
          </div>
        )}
      </div>
      ...
      <div style={{ marginTop: "2rem", padding: "1.1rem 1.25rem", border: `1px solid ${BORDER}`, borderRadius: 8, background: "rgba(255,255,255,0.015)" }}>
        ...
        <ul style={{ margin: 0, paddingLeft: "1.1rem", color: "rgba(255,255,255,0.5)", fontSize: "0.82rem", lineHeight: 1.7 }}>
          <li>Your PDF is analyzed and <strong style={{ color: "rgba(255,255,255,0.7)" }}>never stored</strong> on our servers.</li>
